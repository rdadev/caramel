...
<body class="portal">
    <?php
        ...
        if(isset($_POST["enviar"])) {
            ...
            if(isset($_POST["marketing"])) { $marketing = true; } else { $marketing = false; };
            ...
            if ($cnpj == "") {
                ...
                $altera = $conn -> prepare("UPDATE tbl_lojas SET DS_RAZAO_LOJA = :razao, ID_BANCO = :banco, NR_AGENCIA_LOJA = :agencia, NR_CONTA_LOJA = :conta, ST_MARKETING_LOJA = :marketing, DS_CHAVE_PIX_LOJA = :pix, HR_TEMPO_ENTREGA_LOJA = :tempo_entrega, VL_TAXA_ENTREGA_LOJA = :valor_entrega WHERE ID_LOJA = :id;");
                $altera -> bindValue(":id", $iniget["ID_LOJA"]);
                $altera -> bindValue(":razao", $razao);
                $altera -> bindValue(":banco", $banco);
                $altera -> bindValue(":agencia", $agencia);
                $altera -> bindValue(":conta", $conta);
                $altera -> bindValue(":marketing", $marketing);
                $altera -> bindValue(":pix", $pix);
                $altera -> bindValue(":tempo_entrega", $tempo_entrega);
                $altera -> bindValue(":valor_entrega", $valor_entrega);
                $altera -> execute();

                echo ("<script>Swal.fire({icon: 'success', title: 'Dados alterados', text: 'Os dados da sua loja foram alterados com sucesso!'});</script>");
            };
        }

        $bancos = $conn -> prepare("SELECT * FROM tbl_bancos");
        $bancos -> execute();

        $consulta = $conn -> prepare("SELECT * FROM tbl_lojas, tbl_lojistas WHERE tbl_lojas.ID_LOJA = tbl_lojistas.ID_LOJA AND tbl_lojistas.ID_LOJISTA = :id");
            ...
            <form action="definicoes.php" method="POST">
                ...
                <fieldset>
                    ...
                    <div class="metade left">
                        <label class="lbs" for="id">CNPJ</label>
                        <input name="cnpj" readonly id="cnpj" type="text" class="txti cnpj" placeholder="CNPJ" value="<?php echo($dados["NR_CNPJ_LOJA"]) ?>"/>
                    </div>
                    <div class="metade left">
                        <label class="lbs" for="loja">Razão social</label>
                        <input name="razao" id="razao" type="text" class="txti" placeholder="Razão social" value="<?php echo($dados["DS_RAZAO_LOJA"]) ?>"/>
                    </div>
                </fieldset>

                <fieldset class="mt">
                    <legend><h2>Dados bancários</h2></legend>
                    <div class="metade left">
                        <label class="lbs" for="bancos">Banco</label>
                        <select name="banco" id="banco" class="txti selections">
                            <?php 
                                if($bancos -> rowCount() != 0){ 
                                    while($row = $bancos -> fetch()) {
                                        if ($row["ID_BANCO"] != $dados["ID_BANCO"]){
                                            echo("<option value='".$row["ID_BANCO"]."'>".$row["NR_BANCO"]." - ".$row["NM_BANCO"]."</option>"); 
                                        } else {
                                            echo("<option selected value='".$row["ID_BANCO"]."'>".$row["NR_BANCO"]." - ".$row["NM_BANCO"]."</option>"); 
                                        };
                                    };
                                };
                            ?>
                    ...
                    <div class="metade left">
                        <label class="lbs" for="agencia">Agência</label>
                        <input name="agencia" id="agencia" type="text" class="txti" placeholder="Código da agência + DV" value="<?php echo($dados["NR_AGENCIA_LOJA"]) ?>"/>
                    </div>
                    <div class="metade left">
                        <label class="lbs" for="conta">Conta corrente</label>
                        <input name="conta" id="conta" type="text" class="txti" placeholder="Código da conta + DV" value="<?php echo($dados["NR_CONTA_LOJA"]) ?>"/>
                    </div>
                    <div class="metade left">
                        <label class="lbs" for="conta">Chave PIX</label>
                        <input name="pix" id="pix" type="text" class="txti" placeholder="Digite a chave PIX" value="<?php echo($dados["DS_CHAVE_PIX_LOJA"]) ?>"/>
                    </div>
                </fieldset>
                ...
                <fieldset class="mt">
                    ...
                    <div class="metade left">
                        <label class="lbs" for="agencia">Tempo médio (minutos)</label>
                        <input name="tempo_entrega" id="entrega" type="number" class="txti" placeholder="Tempo de entrega" value="<?php echo($dados["HR_TEMPO_ENTREGA_LOJA"]) ?>"/>
                    </div>
                    <div class="metade left">
                        <label class="lbs" for="conta">Valor da entrega (R$)</label>
                        <input name="valor_entrega" id="valor_entrega" type="number" step="0.1" class="txti" placeholder="Valor (R$)" value="<?php echo($dados["VL_TAXA_ENTREGA_LOJA"]) ?>"/>
                    </div>
                </fieldset>

                <fieldset>
                <legend><h2>Outros</h2></legend>
                    <div class="labelcheck marketing">
                        <p class="left fit">Receber notificações de marketing via email?</p>
                        <?php if($dados["ST_MARKETING_LOJA"] == 1){$status = "checked";}else{$status="";} ?>
                        <label class="switch right">
                            <input name="marketing" type="checkbox" <?php echo $status; ?> />
                            <span class="slider round"></span>
                        </label>
                    </div>
                </fieldset>
                <div class="box inter"> 
                    <input type="submit" name="enviar" class="bta rbtcontent" value="Confirmar"/>
