...
<body class="portal">
    <?php
        if(isset($_POST["enviar"])) {
            ...
            if ($comentario == "") { 
                echo("<script>Swal.fire({icon: 'error', title: 'Erro', text: 'O campo comentário encontra-se vazio. Preencha os campos corretamente.'});</script>");
            } else {
                $subject = "[LEGAL] Solicitação de moderação para o pedido #".$id;
                $message = "
                    Foi solicitada a moderação para o pedido de código #".$id.", o lojista comentou:<br/><br/>
                    ".$comentario.".
                ";
                $headers = "MIME-Version: 1.1\r\n";
                $headers .= "Content-type: text/html; charset=UTF-8\r\n";
                $headers .= 'From: Caramel <info@caramel.com.br>';
                mail("contato@caramel.app.br", $subject, $message, $headers);

                echo ("<script>
                        Swal.fire({
                            icon: 'success',
                            title: 'Moderação solicitada',
                            text: 'Iremos analisar a demanda. Em breve você receberá uma resposta sobre seu pedido.',
        ...
        $avaliacoes = $conn -> prepare("SELECT tbl_avaliacoes.ID_AVALIACAO, tbl_avaliacoes.ID_USUARIO, tbl_avaliacoes.ID_PEDIDO, tbl_avaliacoes.TX_AVALIACAO, tbl_avaliacoes.DT_POSTAGEM_AVALIACAO, tbl_avaliacoes.DS_COMENTARIO_AVALIACAO, tbl_avaliacoes.ID_LOJA, tbl_usuarios.NM_USUARIO FROM tbl_avaliacoes, tbl_usuarios WHERE tbl_avaliacoes.ID_LOJA = :id AND tbl_usuarios.ID_USUARIO = tbl_avaliacoes.ID_USUARIO ORDER BY DT_POSTAGEM_AVALIACAO ASC");
        $avaliacoes -> bindValue(":id", $iniget["ID_LOJA"]);
        $avaliacoes -> execute();
    ?>
    ...
    <dialog id="moderacao" onkeydown="removeBlur()">
        <div class="cabeca">
            <h1>Moderação</h1>
            <button class="mdfechar" type="button" onclick="fecharModal('#moderacao')"><svg xmlns="http://www.w3.org/2000/svg" height="28" viewBox="0 96 960 960" width="28"><path d="m249 873-66-66 231-231-231-231 66-66 231 231 231-231 66 66-231 231 231 231-66 66-231-231-231 231Z"/></svg></button>
        </div>
        <div class="conteudo">
            <form method="POST" action="avaliacoes.php">
                <input name="id" type="text" class="hidden" id="id_avaliacao"/>
                <label>Comentário</label>
                <input name="comentario" type="text" class="txti" placeholder="Digite o que aconteceu"/>
                <p class="obs">Iremos analisar a demanda e se o comentário estiver infringindo as regras da plataforma iremos remover a avaliação</p>
                <input type="submit" name="enviar" class="bta" value="Enviar"/>
            </form>
        </div>
    </dialog>
    ...
    <main class="exibicao">
        <section class="dsh">
            <div class="titulor"><h2>Avaliações</h2></div>
            <div class="filtragem">
                <div class="avaliativo">
                    <label for="classificacao">Classificação</label>
                    <select id="classificacao" class="txti selections">
                ...
                <div class="avaliativo"> 
                    ...
                    <select id="comentarios" class="txti selections">
                        <option>Com comentário</option>
                        <option>Sem comentário</option>
                    </select>
                </div>
                <div class="avaliativo"> 
                    <label for="inicial">De</label>
                    <input type="date" class="txti"/>
                </div>
                <div class="avaliativo"> 
                    <label for="final">Até</label>
                    <input type="date" class="txti"/>
                </div>
                <div class="avaliativo final"> 
                        ...
                        <?php
                            if($avaliacoes -> rowCount() != 0){ 
                                while($avaliacao = $avaliacoes -> fetch()){ 
                                    echo ("<li class='avaliacao'><span><h3 style='float: left'>".strtok(ucfirst(strtolower($avaliacao["NM_USUARIO"])), " ")."</h3><h4 style='float: right; margin-top: 5px'>".date('d/m/Y', strtotime($avaliacao["DT_POSTAGEM_AVALIACAO"]))."</h4></span>");
                                    echo ("<h4>#".$avaliacao["ID_PEDIDO"]."</h4>");
                                    ...
                                    echo ("<p>".$avaliacao["DS_COMENTARIO_AVALIACAO"]."</p>");

                                    echo ("<button type='button' class='detalhar' onclick=\"abrirLateral('#avaliado'); abrirFundo('#moderar'); parametroAvaliacao('".$avaliacao["ID_AVALIACAO"]."', '".$avaliacao["TX_AVALIACAO"]."', '".strtok(ucfirst(strtolower($avaliacao["NM_USUARIO"])), " ")."', '".$avaliacao["DS_COMENTARIO_AVALIACAO"]."', '".$avaliacao["ID_PEDIDO"]."', '".date('d/m/Y', strtotime($avaliacao["DT_POSTAGEM_AVALIACAO"]))."')\">Detalhes</button>");
                                    echo ("</li>");
                                };
                            };
                        ?>
        ...
        <section id="avaliado" class="lateral">
            <div class="fechar"><span>Avaliação</span><button type="button" onclick="fecharLateral()"><svg xmlns="http://www.w3.org/2000/svg" height="28" viewBox="0 96 960 960" width="28"><path d="m249 873-66-66 231-231-231-231 66-66 231 231 231-231 66 66-231 231 231 231-66 66-231-231-231 231Z"/></svg></button></div>
            <div class="conteudo comfundo">
                <h3 id="nome_avaliacao"></h3>
                <p>Nota da avaliação: <b id="nota_avaliacao"></b></p>
                <h5>Comentário</h5>
                <p id="comentario_avaliacao"></p>
                <h5>Detalhes</h5>
                <div class="psoutros">
                    <p class="outro">Pedido <b id="pedido_avaliacao"></b></p>
                    <p class="outro">Postado em <b id="data_avaliacao"></b></p>
                </div>

            </div>
            <div id="moderar" class="fundo">
                <button type="button" class="bta" onclick="abrirModal('#moderacao')">Solicitar moderação</button>
            </div>
        </section>
    </main>
