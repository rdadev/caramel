...
<body class="portal">
    <?php
        $pedidos = $conn -> prepare("SELECT ID_PEDIDO, VL_TOTAL_PEDIDO FROM tbl_pedidos WHERE ID_LOJA = :id AND CAST(DT_TRANSACAO_PEDIDO AS DATE) = CURDATE()");
        $pedidos -> bindValue(":id", $iniget["ID_LOJA"]);
        $pedidos -> execute();

        $avaliacoes = $conn -> prepare("SELECT ID_AVALIACAO, TX_AVALIACAO FROM tbl_avaliacoes WHERE ID_LOJA = :id AND DT_POSTAGEM_AVALIACAO = CURDATE()");
    ...
    <main class="exibicao">
        <section class="dsh">
            <div class="titulor"><h2>Métricas</h2><p>Hoje</p></div>
            <div class="metricas">
                <div class="metrica">
                    <h4>Pedidos</h4>
                    <h3><?php echo($pedidos -> rowCount()) ?></h3>
                    <a class="detalhar" href="pedidos.php">Gerenciar</a>
                </div>
                <div class="metrica mob">
                    <h4>Avaliações</h4>
                    <h3><?php echo($avaliacoes -> rowCount()) ?></h3>
                    <a id="gerava" class="detalhar <?php if($iniget['ID_PERMISSAO_LOJISTA'] == 4 || $iniget['ID_PERMISSAO_LOJISTA'] == 3){echo("hidden");};?>" href="avaliacoes.php">Gerenciar</a>
                </div>
                <div class="metrica">
                    <h4>Média (Avaliações)</h4>
                    <h3>
                        <?php
                            $total = 0;
                            $media = 0;
                    ...
                    <?php
                            ...
                            if($pedidos -> rowCount() != 0){ 
                                while($pedido = $pedidos -> fetch()){ 
                                    $total = $total + $pedido["VL_TOTAL_PEDIDO"];
                                };
                                $media = $total / $pedidos -> rowCount();
                            };
