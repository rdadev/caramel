...
<body class="entrada">
    <?php
        ...
        if(isset($_POST["recuperacao"])) {
            $email = $_POST["email"];
            $recupera = $conn -> prepare("SELECT * FROM tbl_lojistas WHERE DS_EMAIL_LOJISTA = :email");
            ...
            if ($email == "") {
                ...
                if($recupera -> rowCount() == 0) {
                    ...
                    $id = $consulta['ID_LOJISTA'];
                    $nome = $consulta['NM_LOJISTA'];
                    $email = $consulta['DS_EMAIL_LOJISTA'];
                    $rand = mt_rand(1, 999999);
                    $codigo = str_pad($rand, 6, 0, STR_PAD_LEFT);
    
                    $salva_codigo = $conn -> prepare("UPDATE tbl_lojistas SET NR_CONFIRMACAO_LOJISTA = :codigo WHERE ID_LOJISTA = :id");
                    ...
                    $salva_codigo -> execute();
                        ...
                        Olá, ".$nome."!
                        <br/>
                        Recebemos sua solicitação de redefinição de senha, agora é preciso um passo essencial, validar que a alteração está sendo feita por você. Para isso, existe abaixo um código de confirmação que você pode inserir na <a href='https://caramel.app.br/lojista/redefinicao.php'>página de confirmação</a> ou clicando no link abaixo para confirmar automaticamente.
                        <br/><br/>
                        Código: <b>".$codigo."</b>
                        <br/>
                        Link de confirmação: <a href='https://caramel.app.br/lojista/redefinicao.php?email=".$email."&codigo=".$codigo."'>https://caramel.app.br/lojista/redefinicao.php?email=".$email."&codigo=".$codigo."</a>
                        <br/><br/>
                        --
                        <br/>
                        Atenciosamente,
