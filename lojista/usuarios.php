...
<body class="portal">
    <?php
        ...
        $consulta = $conn -> prepare("SELECT ID_LOJA FROM tbl_lojistas WHERE ID_LOJISTA = :id");
        $consulta -> bindValue(":id", $usuario);
        $consulta -> execute();
        $identificacao = $consulta -> fetch();
        $loja = $identificacao["ID_LOJA"];
        ...
        if(isset($_POST["alterar"])) {
            ...
            if ($cpf == "") {
                ...
                echo("<script>Swal.fire({icon: 'error', title: 'Erro', text: 'O campo permissão encontra-se vazio. Preencha os campos corretamente.'});</script>");  
            } else {
                if(isset($_POST["ativo"])) { $ativo = true; } else { $ativo = false; };
                if($id == "") {
                    $consulta_duplicidade = $conn -> prepare("SELECT DS_EMAIL_LOJISTA FROM tbl_lojistas WHERE DS_EMAIL_LOJISTA = :email");
                    $consulta_duplicidade -> bindValue(":email", $email);
                    $consulta_duplicidade -> execute();

                    $consulta_cpf = $conn -> prepare("SELECT NR_CPF_LOJISTA FROM tbl_lojistas WHERE NR_CPF_LOJISTA = :cpf");
                    $consulta_cpf -> bindValue(":cpf", $cpf);
                    $consulta_cpf -> execute();

                    $consulta_qtde = $conn -> prepare("SELECT * FROM tbl_lojistas WHERE ID_LOJA = :id");
                    $consulta_qtde -> bindValue(":id", $loja);
                    $consulta_qtde -> execute();
                        
                    if ($consulta_duplicidade -> rowCount() == 1) {
                        echo ("<script>Swal.fire({icon: 'error', title: 'Cadastro já existente', text: 'Já existe um cadastro com este endereço de e-mail, tente cadastrar outro e-mail.'});</script>");
                    } else if($consulta_cpf -> rowCount() == 1) {
                        echo ("<script>Swal.fire({icon: 'error', title: 'Cadastro já existente', text: 'Já existe um cadastro com este CPF, entre em contato com o suporte.'});</script>");
                    } else if($consulta_qtde -> rowCount() >= 5) {
                        echo ("<script>Swal.fire({icon: 'error', title: 'Limite de cadastros', text: 'O limite de usuários em sua loja foi atingido, entre em contato com o suporte.'});</script>");
                    } else {
                        $senha = mt_rand(1, 999999999);
                        $cripsenha = md5($senha);

                        $cadastro = $conn -> prepare("INSERT INTO tbl_lojistas (NM_LOJISTA, DS_SENHA_LOJISTA, NR_CPF_LOJISTA, DS_EMAIL_LOJISTA, ID_LOJA, ID_PERMISSAO_LOJISTA, ST_CONFIRMACAO_LOJISTA, NR_CONFIRMACAO_LOJISTA, ST_ATIVIDADE_LOJISTA, DS_TEMPMAIL_LOJISTA, ST_TEMPMAIL_LOJISTA, DT_CADASTRO_LOJISTA) VALUES (:nome, :senha, :cpf, :email, :id_loja, :permissao, 1, NULL, :ativo, NULL, 0, :data);");
                        $cadastro -> bindValue(":nome", $nome);
                        $cadastro -> bindValue(":senha", $cripsenha);
                        $cadastro -> bindValue(":cpf", $cpf);
                        $cadastro -> bindValue(":email", $email);
                        $cadastro -> bindValue(":id_loja", $loja);
                        $cadastro -> bindValue(":permissao", $permissao);
                        $cadastro -> bindValue(":ativo", $ativo);
                        $cadastro -> bindValue(":data", date("Y-m-d"));
                        $cadastro -> execute();

                        $subject = "Bem-vindo(a) ao Caramel, aqui está sua senha de acesso!";
                        $message = "
                            Olá, ".$nome."!
                            <br/>
                            Recebemos a solicitação de cadastro feita pelo administrador de sua loja, abaixo encontra-se a senha para o primeiro acesso.
                            <br/><br/>
                            Para alterar sua senha navegue até <b>Conta > Senha > Alterar.</b>
                            <br/>
                            Senha: <b>".$senha."</b>
                            <br/>
                            Link para login: <a href='https://caramel.app.br/lojista/login.php'>https://caramel.app.br/lojista/login.php</a>
                            <br/><br/>
                            --
                            <br/>
                            Atenciosamente,
                        ...
                        Swal.fire({
                            ...
                            text: 'Os dados foram salvos com sucesso, um e-mail contendo a senha de acesso temporária foi enviada para o usuário cadastrado.',
                    ...
                    $consulta_email = $conn -> prepare("SELECT DS_EMAIL_LOJISTA, ST_MASTER_LOJISTA FROM tbl_lojistas WHERE ID_LOJISTA = :id");
                    $consulta_email -> bindValue(":id", $id);
                    $consulta_email -> execute();
                    $cons_email = $consulta_email -> fetch();
                    
                    if ($email != $cons_email["DS_EMAIL_LOJISTA"]) {
                        ...
                        if ($consulta_duplicidade -> rowCount() == 1) {
                            ...
                            $salva_temp = $conn -> prepare("UPDATE tbl_lojistas SET NR_CONFIRMACAO_LOJISTA = :codigo, DS_TEMPMAIL_LOJISTA = :email, ST_TEMPMAIL_LOJISTA = 1 WHERE ID_LOJISTA = :id AND ID_LOJA = :id_loja");
                            $salva_temp -> bindValue(":codigo", $codigo);
                            $salva_temp -> bindValue(":email", $email);
                            $salva_temp -> bindValue(":id", $id);
                            $salva_temp -> bindValue(":id_loja", $loja);
                            $salva_temp -> execute();
                            ...
                            if ($cons_email["ST_MASTER_LOJISTA"] != true) {
                                $altera = $conn -> prepare("UPDATE tbl_lojistas SET NM_LOJISTA = :nome, ID_PERMISSAO_LOJISTA = :permissao, ST_ATIVIDADE_LOJISTA = :ativo WHERE ID_LOJISTA = :id AND ID_LOJA = :id_loja;");
                                $altera -> bindValue(":nome", $nome);
                                $altera -> bindValue(":permissao", $permissao);
                                $altera -> bindValue(":ativo", $ativo);
                                $altera -> bindValue(":id", $id);
                                $altera -> bindValue(":id_loja", $loja);
                                $altera -> execute();
                            } else {
                                $altera = $conn -> prepare("UPDATE tbl_lojistas SET NM_LOJISTA = :nome WHERE ID_LOJISTA = :id AND ID_LOJA = :id_loja;");
                                $altera -> bindValue(":nome", $nome);
                                $altera -> bindValue(":id", $id);
                                $altera -> bindValue(":id_loja", $loja);
                                $altera -> execute();
                        ...
                        echo ("<script>Swal.fire({icon: 'success', title: 'Dados alterados', text: 'Os dados foram alterados com sucesso!'});</script>");
                    };
                };
            };
        };
        
        $dados = $conn -> prepare("SELECT tbl_lojistas.ID_LOJISTA, tbl_lojistas.DS_EMAIL_LOJISTA, tbl_lojistas.NR_CPF_LOJISTA, tbl_permissoes.NM_PERMISSAO_LOJISTA, tbl_lojistas.ID_PERMISSAO_LOJISTA, tbl_lojistas.NM_LOJISTA, tbl_lojistas.ST_ATIVIDADE_LOJISTA FROM tbl_lojistas, tbl_permissoes WHERE tbl_permissoes.ID_PERMISSAO_LOJISTA = tbl_lojistas.ID_PERMISSAO_LOJISTA AND ID_LOJA = :id");
        $dados -> bindValue(":id", $loja);
        $dados -> execute();

        $permissoes = $conn -> prepare("SELECT * FROM tbl_permissoes");
        $permissoes -> execute();
    ?>
    ...
    <main class="exibicao">
        <section class="dsh">
            <div class="titulor"><h2>Usuários</h2></div>
            <div class="acoes">
                <button type="button" class="btb lbtcontent" onclick="abrirLateralUsuarios('#manusuario','','','1','','1','','0');abrirFundo('#fdusuario');">Adicionar usuário</button>
            </div>
            <div class="filtragem">
                <div class="medio">
                    <label for="busca">Busca</label>
                    <input type="text" class="txti" placeholder="Nome do usuário"/>
                </div>
            ...
            <div class="tabelamento">
                <table id="tblusuarios" class="tabela">
                    <thead>
                        <tr class=cabecalho>
                            <th>Usuário</th>
                            <th>Permissão</th>
                    ...
                    <tbody>
                            <?php
                                if($dados -> rowCount() != 0){
                                    ...
                                    echo("<tr>");
                                    echo "<td>".$row["NM_LOJISTA"]."</td><td><div class='contexto'>".$row["NM_PERMISSAO_LOJISTA"]."</div></td><td>".$row["DS_EMAIL_LOJISTA"]."</td><td class='cpf'>".$row["NR_CPF_LOJISTA"]."</td><td><button type='button' class='btlink' onclick='abrirLateralUsuarios(\"#manusuario\",\"".$row["DS_EMAIL_LOJISTA"]."\",\"".$row["NR_CPF_LOJISTA"]."\",\"".$row["ID_PERMISSAO_LOJISTA"]."\",\"".$row["NM_LOJISTA"]."\",\"".$row["ST_ATIVIDADE_LOJISTA"]."\",\"".$row["ID_LOJISTA"]."\",\"1\");abrirFundo(\"#fdusuario\");'>Detalhes</button></td>";
                                    echo("</tr>");
                                    };
                                };
                            ?> 
        ...
        <section id="manusuario" class="lateral">
            <div class="fechar"><span>Cadastro de usuários</span><button type="button" onclick="fecharLateral()"><svg xmlns="http://www.w3.org/2000/svg" height="28" viewBox="0 96 960 960" width="28"><path d="m249 873-66-66 231-231-231-231 66-66 231 231 231-231 66 66-231 231 231 231-66 66-231-231-231 231Z"/></svg></button></div>
            <div class="conteudo comfundo">
                <p class="avs">Lembre-se de informar um email válido, será enviado um email com uma senha temporária para acesso.</p>
                <form class="frmlateral" action="usuarios.php" method="POST">
                    <input name="id" id="id_lojista" class="hidden" type="text"/>
                    <fieldset>
                        <legend><h2>Geral</h2></legend>
                        <div class="totalclass">
                            <label>Email</label>
                            <input name="email" id="usermail" type="text" class="txti" placeholder="email@dominio.com.br" required/>
                        </div>
                        <div class="totalclass">
                            <label>CPF</label>
                            <input name="cpf" id="usercpf" type="text" class="txti cpf" placeholder="CPF" required/>
                        </div>
                        <div class="totalclass">
                            <label>Permissão</label>
                            <select name="permissao" id="permissao" class="txti selections" required>
                                <?php if($permissoes -> rowCount() != 0){ while($row = $permissoes -> fetch()){ echo("<option value='".$row["ID_PERMISSAO_LOJISTA"]."'>".$row["NM_PERMISSAO_LOJISTA"]."</option>"); }; }; ?>
                            </select>
                        </div>
                        <div class="totalclass">
                            <label>Nome</label>
                            <input name="nome" id="username" type="text" class="txti" placeholder="Nome completo" required/>
                            <p class="dica">0/35 caracteres</p>
                        </div>
                    </fieldset>
                    <fieldset class="mt">
                        <legend><h2>Acesso</h2></legend>
                        <div class="labelcheck">
                            <p class="left fit">Ativar usuário?</p>
                    ...
                    <div class="observa">Obs.: O usuário será excluído automaticamente após 90 dias de inatividade.</div>
                    <div class="fundo" id="fdusuario">
                        <input name="alterar" type="submit" class="bta rbtcontent" value="Confirmar"/>
