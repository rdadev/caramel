...
<body class="portal">
    <?php
        $pedidos = $conn -> prepare("SELECT * FROM tbl_pedidos WHERE ID_LOJA = :id AND ST_RECUSA_PEDIDO = 0 ORDER BY DT_TRANSACAO_PEDIDO ASC");
        $pedidos -> bindValue(":id", $iniget["ID_LOJA"]);
        $pedidos -> execute();
    ?>
    ...
    <main class="exibicao">
        <section class="dsh">
            <div class="titulor"><h2>Pedidos</h2></div>
            <div class="filtragem">
                <div class="pedidors">
                    <label for="classificacao">Status</label>
                    <select id="classificacao" class="txti selections">
                        <option value="" selected>Todos</option>
                        <option value="Confirmar">Confirmação</option>
                        <option value="Preparando">Preparação</option>
                        <option value="Entregando">Rota de entrega</option>
                        <option value="Concluído">Concluído</option>
                    </select>
                </div>
                <div class="pedidors"> 
                    <label for="comentarios">Logística</label>
            ...
            <div class="tabelamento">
                <table id="tblpedidos" class="tabela">
                    <thead>
                        <tr class=cabecalho>
                            <th>Pedido</th>
                            <th>Data</th>
                        ...
                        <?php
                            if ($pedidos ->rowCount() > 0) {
                                while($pedido = $pedidos -> fetch()){ 
                                    ...
                                    if ($pedido["ST_PEDIDO"] == 1) {
                                        $classe = "confirmar";
                                        $status = "Confirmar";
                                    } else if ($pedido["ST_PEDIDO"] == 2) {
                                        $classe = "andamento";
                                        $status = "Preparando";
                                    } else if ($pedido["ST_PEDIDO"] == 3) {
                                        $classe = "entrega";
                                        $status = "Entregando";
                                    } else {
                                        $classe = "concluido";
                                    ...
                                    if ($pedido["NR_TIPO_ENTREGA_PEDIDO"] == 1) {
                                        $entrega = "Entrega";
                                    } else {
                                        $entrega = "Retirada";
                                    };
                                    ...
                                    <td>".date('d/m/Y H:i', strtotime($pedido["DT_TRANSACAO_PEDIDO"]))."</td>
                                    <td><div class='".$classe."'>".$status."</div></td>
                                    <td>".$entrega."</td>
                                    <td>".$pedido["NM_BAIRRO_PEDIDO"]."</td>
                                    <td><form method='POST' action='cupom.php'><input type='text' value='".$pedido["ID_PEDIDO"]."' class='hidden' name='id'/><input type='submit' name='detalhar' class='btlink' value='Detalhes'/></form></td>
                                    ");
                                    echo ("</tr>");
                                };
                            };
