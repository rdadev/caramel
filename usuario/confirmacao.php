    ...
    <?php
        ...
        if(isset($_POST["confirmar"])) {
            ...
            if ($codigo == "") {
                ...
                $confirmacao = $conn -> prepare("SELECT * FROM tbl_usuarios WHERE DS_EMAIL_USUARIO = :email");
                $confirmacao -> bindValue(":email", $email);
                $confirmacao -> execute();
    
                if ($confirmacao -> rowCount() == 1) {
                    $consulta = $confirmacao -> fetch();
                    if ($consulta["ST_CONFIRMACAO_USUARIO"] != true && $consulta["NR_CONFIRMACAO_USUARIO"] == $codigo) {
                        $executa = $conn -> prepare("UPDATE tbl_usuarios SET ST_CONFIRMACAO_USUARIO = 1, NR_CONFIRMACAO_USUARIO = NULL WHERE ID_USUARIO = :id;");
                        $executa -> bindValue(":id", $consulta["ID_USUARIO"]);
                        $executa -> execute();
                        echo ("<script>
                        Swal.fire({
                            icon: 'success',
                    ...
                    } else if ($consulta["ST_CONFIRMACAO_USUARIO"] == true) {
                    ...
                    } else if ($consulta["NR_CONFIRMACAO_USUARIO"] != $codigo) {
